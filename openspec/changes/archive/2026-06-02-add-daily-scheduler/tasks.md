...
- **背景**: 前序删除了多个已弃用的内网集成模块,环境模板与依赖清单里仍残留对应配置项与包。
- **操作**:
  - `.env.example`:删除已退役内网集成相关的所有配置项与注释。
  - `requirements.txt`:删除 `cryptography`、`playwright`、`markdown` 等仅服务于已弃用模块的依赖 — 已用 `grep -rn` 确认 `src/` 与 main 入口零引用。
- **验证**:
  - `grep -ni "deprecated\|legacy" .env.example` 无输出。
