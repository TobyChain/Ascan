...
class LLMClient:
    ...
    def __init__(
        ...
        self.api_key = api_key or settings.idealab_api_key
        self.base_url = (base_url or settings.idealab_base_url).rstrip("/")
        ...
        if not self.api_key:
            raise RuntimeError(
                "未配置 IDEALAB_API_KEY，请在 .env 中设置。"
            )

    def _call_api(self, messages: List[Dict[str, str]]) -> str:
        """调用 IdealAb OpenAI 兼容 API，返回 LLM 输出文本。"""
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 429:
                    wait = 2 ** attempt + 1
                    logger.warning(f"IdealAb 限流 429, 等待 {wait}s 后重试 ({attempt + 1}/{self.max_retries})")
                    _time.sleep(wait)
                ...
                if "429" not in str(exc):
                    logger.warning(
                        f"IdealAb API 调用失败 (尝试 {attempt + 1}/{self.max_retries}): {exc}"
                    )

        self.error_count += 1
        raise RuntimeError(
            f"IdealAb API 在 {self.max_retries} 次尝试后仍然失败: {last_error}"
        ) from last_error
    ...
    def _create_analysis_prompt(self, title: str, abstract: str) -> str:
        ...
        return (
            ...
            "core_recommendation=Chinese explanation of how this paper can inspire or benefit e-commerce AI applications (电商AI应用) including insights on digital worker/product recommendation/product inspection/e-commerce customer service/agent framework/intent understanding (30-80 chars), "
            "keywords=list of 3-5 Chinese keywords, "
            "sub_topic=research field in Chinese (e.g. 数字员工/商品推荐/商品巡检/电商客服/智能体框架/意图理解), "
