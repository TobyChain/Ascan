...
class FetchStage(PipelineStage):
    ...
    async def execute(self, context: PipelineContext) -> bool:
        ...
        for subject in context.subjects:
            logger.info(f"抓取主题: {subject} (使用 arxiv API)")
            retry_delays = [5, 10, 15]
            api_success = False

            for attempt, wait in enumerate([0] + retry_delays):
                if wait:
                    logger.warning(f"主题 {subject} 等待 {wait}s 后重试 (第{attempt}次)...")
                    await asyncio.sleep(wait)
                try:
                    date_compact = context.date.replace("-", "")
                    query = f"cat:{subject} AND submittedDate:[{date_compact}0000 TO {date_compact}2359]"
                    client = _configure_arxiv_client(
                        arxiv.Client(num_retries=1, delay_seconds=3),
                    )
                    search = arxiv.Search(
                        query=query,
                    ...
                    logger.info(
                        f"主题 {subject}: 找到 {len(subject_results)} 篇，累计去重后 {len(all_ids)} 篇"
                    )
                    api_success = len(subject_results) > 0
                    break
                except Exception as e:
                    if "429" in str(e) and attempt < len(retry_delays):
                        logger.warning(f"主题 {subject} 遭遇限流 (429): {e}")
                        continue
                    else:
                        logger.warning(f"主题 {subject} API 抓取失败: {e}")
                        break

            if not api_success:
                logger.info(f"主题 {subject}: API 返回 0 篇或不可用，尝试 RSS fallback...")
                try:
                    r = _requests.get(f"https://rss.arxiv.org/rss/{subject}", verify=False, timeout=30)
                    if r.status_code == 200:
                        ...
                        logger.info(f"主题 {subject} RSS fallback: 获取 {rss_count} 篇，累计 {len(all_ids)} 篇")
                    else:
                        logger.warning(f"主题 {subject} RSS 也失败: HTTP {r.status_code}")
                except Exception as e2:
                    logger.warning(f"主题 {subject} RSS fallback 异常: {e2}")
...
class ParseStage(PipelineStage):
    ...
    def _fetch_arxiv_metadata(self, ids: list[str], batch_size: int = 100) -> list:
        ...
        client = _configure_arxiv_client(arxiv.Client())
        all_results = []
...
class UploadStage(PipelineStage):
    """上传阶段（飞书）— 从原 arXiv-ai-Agent 保留的上传接口"""
    ...
    async def execute(self, context: PipelineContext) -> bool:
        try:
            ...
            if not getattr(settings, "enable_feishu_push", False):
                logger.info("飞书推送已禁用")
                return True

            if not hasattr(context, "report_path") or not getattr(context, "report_path", None):
                logger.error("报告路径不存在")
                return False

            folder_token = getattr(settings, "feishu_docx_folder_token", None)
            if not folder_token:
            ...
            if document_id:
                base_url = getattr(settings, "feishu_docx_base_url", "").rstrip("/")
                context.report_url = f"{base_url}/{document_id}" if base_url else document_id
...
class NotifyStage(PipelineStage):
    """通知阶段（飞书卡片）— 从原 arXiv-ai-Agent 保留的通知接口"""
    ...
    async def execute(self, context: PipelineContext) -> bool:
        try:
            ...
            if not getattr(context, "report_url", None):
                logger.warning("报告 URL 不存在，跳过通知")
                return True

            with get_db_session() as db:
                repo = PaperRepository(db)
                stats = repo.get_statistics(context.date) if hasattr(repo, "get_statistics") else {}

            notifier = FeishuNotifier()
            success = notifier.send_daily_report(
                date=context.date,
                paper_count=stats.get("total", 0) if isinstance(stats, dict) else 0,
